...
function drawBubble(data) {
  ...
  data = data.map(function(d){ d.id = d.ISO; return d});
  ...
  var rescale = 1, 
    ...
    // viridis
    //.range(["#FDE725", "#AADC32", "#5DC863", "#27AD81", "#21908C", "#2C728E", "#3B528B", "#472D7B", "#440154"]);    
    // magma
  ...
  .on('mouseover', function(d) {      
    ...
    .duration(0)      
    .style('opacity', 1);      
    div.html('<b><font size = "3">' + d.name + '</font></b>' + '<br/>' + d.region  + '<br/>' +'Imports: $' + format(d.value) + 'bn' +'<br/>'+ 'Average applied tariffs: ' + format(d.tariff) + '%')
    .style('left', 700 + 'px') 
    .style('top', 134 + 'px'); 
    d3.select(this)
    .transition()
  ...
  .on('mousemove', function(d) {      
    ...
    format(d.value) + 'bn' +'<br/>'+ 'Average applied tariffs: ' + format(d.tariff) + '%')
    .style('left', 700 + 'px') 
    .style('top', 134 + 'px'); 
    d3.select(this)
    .transition()
...
function updateData() {
  var newArray = d3.select("#optsBubble").node().value;
  var data = dataBubble[newArray];
  d3.select("#bubbleSvg").remove();
  d3.select("#bubbleDiv").remove();
  drawBubble(data);
}

d3.select('#optsBubble')
