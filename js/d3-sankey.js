...
// Modifications by Marc Gumowski:
// - Modified functions at lines 214 and 232 to order the nodes by value.
// - Added manually '- height / 2)' at line 37 to place center more or less the alignement of the nodes. 
// - Added '+ py' at line 250 to center the node.
// - Changed manually 'alpha *= 1' at line 194 to match perfeclty the ordering of the nodes. 
...
function nodeCenter(node) {
  return node.y + node.dy / 2; // - (height / 2); // Added - (height / 2)
}
...
var sankey = function() {
  ...
  function computeNodeDepths(graph) {
    ...
    for (var alpha = 1, n = iterations; n > 0; --n) {
      relaxRightToLeft(alpha *= 1); // Previously alpha *= 0.99, change slightly the ordering
      resolveCollisions();
    ...
    /* MODIFIED FROM ORIGINAL - Download d3-sankey.js to have original one */
    ...
    function resolveCollisions() {
      nodesByBreadth.forEach(function(nodes) {
        ...
        nodes.sort(ascendingDepth); // Comment this line to stop sorting
        for (i = 0; i < n; ++i) {
          node = nodes[i];
          dy = y - node.y;
          if (dy > 0) node.y += dy;
